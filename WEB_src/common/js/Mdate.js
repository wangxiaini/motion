        ...
        Mdate.prototype = {
            ...
            dateBoxShow: function() {
                ...
                that.selectorId.onclick = function() {
                    ...
                    // if(that.selectorId.id === 'dateShowBtn'){
                    //     sureBtn.onclick=function () {
                    //         console.log(1)
                    //     }
                    //     // sessionStorage.setItem('time1',that.selectorId.childNodes.value)
                    // }else{
                    //     sureBtn.onclick=function () {
                    //         alert(2)
                    //     }
                    //     // sessionStorage.setItem('time2',that.selectorId.childNodes.value)
                    // }
                }
            },
            ...
            initScroll: function() {
                ...
                yearScroll = new iScroll("yearwrapper",{
                    ...
                    onScrollEnd: function() {
                        indexY = Math.ceil(this.y / 80 * -1 + 1);
                ...
                monthScroll = new iScroll("monthwrapper",{
                    ...
                    vScrollbar: false,
                    onScrollEnd: function() {
                        indexM = Math.ceil(this.y / 80 * -1 + 1);
                ...
                dayScroll = new iScroll("daywrapper",{
                    ...
                    vScrollbar: false,
                    onScrollEnd: function() {
                        indexD = Math.ceil(this.y / 80 * -1 + 1);
            ...
            refreshScroll: function() {
                ...
                inputDay -= initD;
                yearScroll.refresh();
                yearScroll.scrollTo(0, inputYear * 80, 300, true);
                monthScroll.scrollTo(0, inputMonth * 80, 300, true);
                dayScroll.scrollTo(0, inputDay * 80, 300, true)
            },
            dateSure: function() {
                var that = this;
                var sureBtn = d.getElementById("dateSure");
                ...
                sureBtn.onclick = function() {
                    ...
                    that.acceptId.setAttribute("value", yearTag + that.format + that.dateForTen(monthTag) + that.format + that.dateForTen(dayTag));
                    that.time = yearTag + that.format + that.dateForTen(monthTag) + that.format + that.dateForTen(dayTag)
                    that.dateCancel()
                    if(that.selectorId.id === 'dateShowBtn'){
                        sessionStorage.setItem('time1',that.time)
                    }else{
                        sessionStorage.setItem('time2',that.time)
                    }
                }
                ;
