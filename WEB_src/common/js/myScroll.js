Vue.component('my-scroll', {
  ...
  template: `<div class="yo-scroll" :class="{'down':(state===0),'up':(state==1),refresh:(state===2),touch:touching}" @touchstart="touchStart($event)" @touchmove="touchMove($event)" @touchend="touchEnd($event)">
              <section class="inner" :style="{ transform: 'translate3d(0, ' + top + 'px, 0)' }">
                  ...
                  <footer class="load-more" v-show="hasbe">
                      <slot name="load-more">
                          <span v-show="hasbe">加载中……</span>
                      </slot>
                  </footer>
                  <div class="nullData" v-show="hasend">已无更多数据</div>
              </section>
            </div>`,
            props: {
              offset: {
          ...
          methods: {
              ...
              touchMove(e) {
                  if(!this.enableRefresh || this.hasend || !this.touching || !this.hasbe) {
                    e.stopPropagation()
                      return false
                  }
